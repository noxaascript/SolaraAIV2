...
- gemini-3-pro-image (nano banana pro): thinking model that is good for high-quality image generation tasks where detail and accuracy matters most.
...
The callback derives and sets `AI_INTEGRATIONS_GEMINI_BASE_URL` and `AI_INTEGRATIONS_GEMINI_API_KEY`.
...
```javascript
const geminiResult = await setupReplitAIIntegrations({
  providerSlug: "gemini",
});
...
- `generateImage(prompt)` - Generates an image using Gemini's native image generation and returns `{ b64_json, mimeType }` - this is an app helper exported by `@workspace/integrations-gemini-ai/image`, not the agent PTC callback from the `media-generation` skill.
...
This `generateImage(prompt)` call is the app helper imported above. It is not the agent `media-generation` skill ptc `generateImage` callback.
