...
- gpt-5.6-sol: most powerful model in the gpt-5.6 fleet, but also the most expensive. Use for the hardest tasks when capability matters more than cost.
- gpt-5.6-terra: best general-purpose model. Prefer for most tasks, including coding.
- gpt-5.6-luna: affordable model in the gpt-5.6 fleet. Use for cost-sensitive or high-volume tasks.
- gpt-5.4: highly capable general-purpose model, prefer gpt-5.6-terra for non-coding tasks.
- gpt-5.3-codex: specialized coding model; prefer gpt-5.6-terra for most coding tasks. Responses API only (not chat completions).
- gpt-5.2: good for most tasks, prefer gpt-5.6-terra
- gpt-5.2-codex: code-optimized gpt-5.2 variant. Responses API only (not chat completions).
- gpt-5.1, gpt-5: good for most tasks, prefer gpt-5.6-terra
...
- gpt-audio: speech-to-speech (text/audio in, audio+transcript out). Best for speech-to-speech with no TTS chaining. Less intelligent than newer text models (e.g. gpt-5.6-terra).
