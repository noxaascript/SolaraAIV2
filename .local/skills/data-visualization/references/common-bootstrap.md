...
1. **User mentioned specific sources** (external databases, APIs, services):
   ...
   - If not set up: `ProposeIntegration({ proposal: [{ integrationId: "connector:ccfg_..." }] })`
...
2. **No data source specified**: If the data visualization artifact setup question already asked for sources, use its answer. Otherwise, you can use `searchIntegrations()` to look up available connectors and suggest them to the user.
