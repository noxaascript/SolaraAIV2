...
from core.hf_credentials import has_user_key, set_user_key
...
def chat_loop(state):
    ...
    while True:
        ...
        if text in ("/exit", "exit", "quit"):
        ...
        elif text == "/sethfkeyauto":
            try:
                key = getpass.getpass("  Hugging Face token (hidden): ").strip()
                if set_user_key(username, key):
                    system_msg("Hugging Face key loaded for this session.")
                else:
                    error_msg("Invalid key. Hugging Face tokens normally start with hf_.")
            except (KeyboardInterrupt, EOFError):
                error_msg("Key setup cancelled.")

        elif text in ("/hfstatus", "/key"):
            system_msg(
                "Hugging Face key is configured for this session."
                if has_user_key(username)
                else "No user Hugging Face key is configured."
            )
            ...
            try:
                if state.get("auto"):
                    ...
                    response = run_ai(state["model"], text, user_id=username)
