...
from core.code_output import save_generated_code, save_generated_code_with_metadata
...
class TestCodeOutput(unittest.TestCase):
    ...
    def test_does_not_save_a_folder_tree_as_source_code(self):
        response = "```tree\ngenerated_code/\n└── app.py\n```"
        with tempfile.TemporaryDirectory() as directory:
            with patch("core.code_output.OUTPUT_DIR", Path(directory) / "generated_code"):
                result, path = save_generated_code_with_metadata("make an app", response)
